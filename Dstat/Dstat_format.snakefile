...
def combine_pops(pops_h1, pops_h2, pops_h3, outgroup, output):
    ...
    with open(output, 'w') as outFile:
        [outFile.write("\t".join([combination[0], combination[1], pop3])+"\n") 
...
rule run_qpDstat:
    input:
        ...
        pop_file = "{panel}.combinations"
    output:
        result = "Results/{myRange}/{panel}_{myRange}.qpDstat.results" # expand_range("{panel}")
    shell:
        """
...
rule wrap_qpDstat:
    input:
        dstat = expand("Results/{myRange}/{panel}_{myRange}.qpDstat.results", 
                        panel = panel, myRange = expand_range(panel))
    output:
        result = "Results/{panel}_qpDstat.results"

    shell:
        """
            cat {input.dstat} > {output.result}
        """
