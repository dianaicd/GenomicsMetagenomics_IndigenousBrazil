...
median_age <- sapply(carbon$Ethnicity, function(x) median(carbon$Age..BP.[carbon$Ethnicity == x]))
#m <- m[!is.na(m)]
median_age <- median_age[order(median_age)]
names_age <- unique(names(median_age))
median_age <- unique(median_age)
names(median_age) <- names_age
...
carbon$ID <- factor(carbon$ID, levels = unlist(
  sapply(names(median_age), 
         function(x)
           carbon$ID[carbon$Ethnicity==x])))

#id <- sapply(seq(1,14), function(x) carbon$ID[carbon$Ethnicity == names(m[x])])

mn_id <- unlist(sapply(seq(1,length(median_age)),
              function(x) {
                y <- carbon[carbon$Ethnicity == names(median_age[x]), c("ID", "Age..BP.")]
                y <- y[order(y$Age..BP.),]
                return(y$ID)
               }))

carbon$ID <- factor(carbon$ID, levels = mn_id
                   )
...
eth <- data.frame(ID = mn_id
                 [round(sapply(
  unique(carbon$Ethnicity),
  function(x) 
    median(which(
      carbon$Ethnicity == x))))],
  Ethnicity = unique(sapply(mn_id
                           , function(x) carbon$Ethnicity[carbon$ID == x])),
  Age..BP. = sapply(names(median_age),
...
other <- ggplot(carbon[!grepl("Boto.*", carbon$Ethnicity),], aes(x = ID,
                                                              y = Age..BP.,
                                                              ymin = Age..BP. - SD,
                                                              ymax = Age..BP. + SD,
                                                              color = Ethnicity, group = Ethnicity)) +
...
22 individuals were dated at ^14^CHRONO Center at Queen's University Belfast.
...
```{r table1, echo = F}
myT1 <- carbon[,c("ID", "Material_type", "Age..BP.", "SD", "Ethnicity",
                  "d13C", "Endogenous","d15N", "C.N.ratio")]
colnames(myT1) <- c("ID", "Material type", "Age (BP)", "SD", 
                 "Details", "Endogenous DNA", "d13C", "d15N", "C:N ratio")
myT1 <- myT1[order(myT1$`Age (BP)`),]
rownames(myT1) <- seq(1, length(myT1$ID))
myT1 <- xtable(myT1, caption = "Table 1. Isotope valules and calibrated dates.")

kable(myT1, caption = "Table 1. Isotope values and uncalibrated dates.")
```
...
We measured the $\delta$^13^C and $\delta$^15^N values for the 22 individuals. 
...
carbon$child <- child
d <- carbon[child, ] #carbon$ID %in% c("MN00022", "MN00045"),]
...
ggplot(carbon, aes(x = d13C, y = d15N, color = fmolar,
  ...
  coord_flip() +
  geom_text(data = d, aes(x = d13C-.5, y = d15N+0, label = ID))
...
kinaston <- read.csv("~/Projects/Botocudos/Files/Isotopic/Kinaston2014.csv")
...
ind$color <- "midnightblue"
ind$color[ind$Cod %in% c("Bot15", "Bot17")] <- "orange1"
ind$color[ind$Code == "Sambaqui_Cabecuda"] <- "brown1"
ind$color[ind$Code == "NativeAmerican_MorrodaBabilonia"] <- "chartreuse"
points(x = ind$d15N, y = ind$d13C, xlim = c(0,33),
     ylim = c(-22, -3), #type = "n",
     pch = shapes, col = ind$color)
...
ggplot(table_s1[order(table_s1$Mean),], aes(y = Mean, x = ID)) + 
