...
mySamples <- read.table(sm)
nInd <- nrow(mySamples)
trimmed <- read.table(error, skip = 0, header = TRUE, nrows = nInd)
...
trimmed$Sample <- rownames(trimmed)

dat <- rbind(whole, trimmed)
dat <- dat[order(dat$Sample),]
dat <- dat[!dat$Sample %in% c("MN0008_L3U", "MN0008_non_U"),]
```


```{r plot untrimmed vs trimmed data, fig.width=20, fig.height=10}
...
```{r compare trimmed untrimmed type specific}
