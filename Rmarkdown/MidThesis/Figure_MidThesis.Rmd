...
myLegend$language <- factor(myLegend$language, levels = c("USR1",
                                                          "NaDene", "EskimoAleut", "Saqqaq",
                                                          "TrailCreek", "SpiritCave", "Clovis", "Lovelock",
                                                          "NorthernAmerind", "SouthWesternOntario",
                                                          "CentralAmerind", "ChibchanPaezan", "Belize",
                                                          "Andean", "EarlyAndes", "LateCentralAndes", "Southern_Cone", "HistoricalAndean",
                                                          "EcuatorialTucanoan", "Ge-PanoCaribean",
                                                          "Brazil", "LagoaSanta", "Taino", 
                                                          "Aconcagua", "PuntaSantaAna", "AncientYamana",
                                                          "Ayayema", "AncientKaweskar", "Botocudos"))
...
```{r make animated gift}


ggplot(pointsWColor, aes(x = pointsWColor[,1]/2e5,
                         y = pointsWColor[,2]/2e5,
                         color = language,
                         shape = language,
                         fill = language)) +
  geom_point(size = 5) +
  scale_color_manual(values = myLegend$color,
                     breaks = myLegend$language,
                     labels = myLegend$language) +
  scale_shape_manual(values = myLegend$shape,
                     breaks = myLegend$language,
                     labels = myLegend$language) +
  scale_fill_manual(values = myLegend$bg, 
                    breaks = myLegend$language,
                    labels = myLegend$language) +
  theme_minimal() +
  labs(title = "MDS, Native Americans", x = NULL, y = NULL)
...
myAes.long <- reshape(pointsWColor,
                      v.names = "coordinate",
                      idvar = "id",
                      varying =  paste(as.character(seq(1,nrow(pointsWColor)+1)), sep = ""),
                      direction = "long")
```
...
p <- ggplot(myAes_time, aes(x = coordinate/2e5,
                         y = coordinate2/2e5,
                         color = language,
                         shape = language,
                         fill = language)) +
  ...
  scale_fill_manual(values = myLegend$bg, 
                    ...
                    name = NULL) +
  theme_minimal() +
  labs(title = "MDS, Native Americans\n(dim. 1- 10)", x = NULL, y = NULL)
  
  
  # ggplot(myAes_time, aes(x = coordinate/2e5, y = coordinate2/2e5,
  #                           color = legend)) +
  # geom_point(size = 5) +
...
y <- animate(x, renderer = gifski_renderer(loop = F),
             nframes = max(all_coor)*6,
             width =  7, height = 4, res = 200, units = "in")
anim_save("myGif.gif", y, path = "~/")
...
png("~/Projects/Botocudos/Plots/Map/MDS_DBC_map.png", width = 10, height = 6,
    res = 300, units = "in")

layout(matrix(c(1,2), nrow = 1), widths = c(1,1), heights = c(1))
par(fg = NULL)
