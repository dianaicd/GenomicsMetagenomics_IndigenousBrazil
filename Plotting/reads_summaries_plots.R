...
summary_reads_plot <- function(boto){
  ...
  ggplot(boto[boto$Library != "*", ], aes(x = Library)) +
    ...
    geom_bar(aes(y = seq_retained_reads, fill = "seq_retained_reads"), 
             stat = "identity") +
    geom_bar(aes(y = hits_unique.nuclear., fill = "hits_unique.nuclear."), 
             stat = "identity") +
    ...
    annotate("text", x = 17, y = 1.1*m,#3.5e+7,
             label = "5% - 10%") +
    annotate("text", x = 22.5, y = 1.1*m,#3.5e+7,
             label = "16% - 18%") +
...
hg_mapping_plot <-function(boto, ylim = F){
  if(!is.numeric(ylim)){
    m <- max(boto$hits_unique.nuclear.)
    ylim <- 1.3*m
  }else{
    m <- ylim/1.3
  }
