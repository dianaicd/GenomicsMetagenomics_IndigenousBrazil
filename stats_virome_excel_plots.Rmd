...
```{r}
plot_cov <- function(coverage, plot.title, myColor){
...
for (s in selected_samples) {
  for (ref in sorted_refs) {
    ...
    some_cov <- list(coverage = tmp_cov)
    myCov[[ref]] <- some_cov
  }
  
  png(paste0("~/axiom_virome/Boto_virome/true_complete/virus_mappings/parvovirus/single_fasta/output_mystats/all_samples/coverage_plots/selected_samples/quality_", q, "/", s, "_mapQ", q, ".png"), width = 1024, height = 768)
  layout(matrix(seq(nCol*nRow), ncol = nCol, byrow = T))
  
  for (ref in sorted_refs) {
    plot_cov(coverage = myCov[[ref]]$coverage$reads, 
...
# par(xpd = T)
# barplot(myCov$MN00346$nb_reads)
# rect(xleft = 3012, xright = 4676, ybottom = -5, ytop = -1, col = "#d60036", border = NA)
```
